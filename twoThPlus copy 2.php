...
<table>
<tr>
<th>Make</th><th>Model</th><th>Reg</th><th>Price</th><th>Colour</th><th>Miles</th>
</tr>
<?php

require 'databaseTemplate.php';

$stmt = $pdo->query('SELECT * FROM cars Where miles=51000 or miles<51000 ORDER BY miles ASC');

while($row = $stmt->fetch())
{
    echo "<tr>";
    echo "<td>" . $row['make']."</td><td>". $row['model']. 
    "</td><td>". $row['Reg']. "</td><td>". $row['price']. "</td><td>". $row['colour'] ."</td><td>". $row['miles'] ."</td>";
    echo "</tr>";
}
